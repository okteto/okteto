...
// TestUpComposeWithGateway tests okteto up with compose using Gateway API HTTPRoute
// It verifies that when OKTETO_COMPOSE_ENDPOINTS_TYPE=gateway, HTTPRoute resources are created instead of Ingress
// Note: Cannot use t.Parallel() because t.Setenv() modifies process-wide environment
func TestUpComposeWithGateway(t *testing.T) {
	// Set environment variable to force Gateway API HTTPRoute
	t.Setenv("OKTETO_COMPOSE_ENDPOINTS_TYPE", "gateway")
	...
	log.Printf("✓ HTTPRoute 'nginx' exists in namespace '%s'", testNamespace)

	// Verify Ingress does NOT exist
	...
	log.Printf("✓ Ingress 'nginx' does not exist (expected)")
