...
// Note: this test requires two variables configured in the Okteto UI Admin Variables:
// 'CLI_TEST_MY_VAR4=admin' and 'CLI_TEST_MY_VAR5=admin'
// This test also validates that local variables are supported when the feature flag is enabled
func TestDeployVariablesOrderWithLocalVarSupportEnabled(t *testing.T) {
	t.Setenv("CLI_TEST_MY_VAR3", "local")
	t.Setenv("OKTETO_SUPPORT_LOCAL_VARIABLES_ENABLED", "true")
