...
// TestBuildWithNestedCompose tests the following scenario:
// - Build
...
func TestBuildWithNestedCompose(t *testing.T) {
	t.Parallel()
	...
	composeAppDir := filepath.Join(dir, "compose")
	require.NoError(t, os.Mkdir(appDir, 0700))
...
func createComposeFile(dir string) error {
	composeFilePath := filepath.Join(dir, "docker-compose.yaml")
	composeFileContent := []byte(composeWithCustomImage)
	if err := os.WriteFile(composeFilePath, composeFileContent, 0600); err != nil {
		return err
	}
	return nil
}
