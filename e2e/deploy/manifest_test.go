...
func TestDeployOktetoManifest(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestDeployManifestV2", user)
...
func TestRedeployOktetoManifestForImages(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestReDeploy", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
...
func TestDeployOktetoManifestWithDestroy(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestDeployDestroy", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
	...
	_, err = e2e.GetConfigmap(context.Background(), testNamespace, fmt.Sprintf("okteto-git-%s", filepath.Base(dir)), c)
	require.NoError(t, err)
...
func TestDeployOktetoManifestExportCache(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestDeployExportCache", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
...
func TestDeployRemoteOktetoManifest(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestDeployRemote", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
	...
	_, err = e2e.GetDeployment(context.Background(), testNamespace, "my-dep", c)
	require.True(t, k8sErrors.IsNotFound(err))
}
