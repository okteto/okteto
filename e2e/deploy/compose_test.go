...
func TestDeployPipelineFromCompose(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestDeployCompose", user)
	...
	nginxDeployment, err := e2e.GetDeployment(context.Background(), testNamespace, "nginx", c)
	require.NoError(t, err)
	...
	// Test that the nginx image has been created correctly
	appDeployment, err := e2e.GetDeployment(context.Background(), testNamespace, "app", c)
	require.NoError(t, err)
	...
	appService, err := e2e.GetService(context.Background(), testNamespace, "app", c)
	require.NoError(t, err)
	require.Len(t, appService.Spec.Ports, 3)
	for _, p := range appService.Spec.Ports {
		require.Contains(t, []int32{8080, 8913, 2931}, p.Port)
	}
	nginxService, err := e2e.GetService(context.Background(), testNamespace, "nginx", c)
	require.NoError(t, err)
	...
	require.NotEmpty(t, e2e.GetContentFromURL(nginxURL, timeout))
	...
	_, err = e2e.GetService(context.Background(), testNamespace, "app", c)
	require.NoError(t, err)

	_, err = e2e.GetService(context.Background(), testNamespace, "nginx", c)
...
func TestReDeployPipelineFromCompose(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestReDeployCompose", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
...
func TestDeployPipelineFromComposeOnlyOneSvc(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestDeployPartialCompose", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
	...
	// Test that the nginx image has been created correctly
	_, err = e2e.GetDeployment(context.Background(), testNamespace, "nginx", c)
	require.True(t, k8sErrors.IsNotFound(err))
...
func TestDeployPipelineFromOktetoStacks(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestDeployStacks", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
...
func TestDeployComposeFromOktetoManifest(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestDeployComposeManifest", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
