...
func TestUpCompose(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestUpCompose", user)
	...
	originalDeployment, err := e2e.GetDeployment(context.Background(), testNamespace, "app", c)
	require.NoError(t, err)
	...
	require.Contains(t, e2e.GetContentFromURL(varRemoteEndpoint, timeout), "value1")

	// Test that the same content is on the remote and on local endpoint
	require.Equal(t, e2e.GetContentFromURL(indexRemoteEndpoint, timeout), testNamespace)
	...
	require.NoError(t, e2e.DestroyPod(context.Background(), testNamespace, "stack.okteto.com/service=app", c))
