...
func TestUpStatefulsetV1(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestUpStatefulsetV1", user)
	...
	require.NoError(t, e2e.WaitForStatefulset(kubectlBinary, kubectlOpts, timeout))

	originalStatefulSet, err := e2e.GetStatefulset(context.Background(), testNamespace, "e2etest", c)
	require.NoError(t, err)
	...
	require.Equal(t, e2e.GetContentFromURL(varLocalEndpoint, timeout), "value1")

	// Test that the same content is on the remote and on local endpoint
	require.NotEmpty(t, e2e.GetContentFromURL(indexLocalEndpoint, timeout))
	require.Equal(t, e2e.GetContentFromURL(indexLocalEndpoint, timeout), testNamespace)
	require.Equal(t, e2e.GetContentFromURL(indexLocalEndpoint, timeout), e2e.GetContentFromURL(indexRemoteEndpoint, timeout))
	...
	sfs, err := e2e.GetStatefulset(context.Background(), testNamespace, "e2etest", c)
	require.NoError(t, err)
	...
	require.NoError(t, e2e.UpdateStatefulset(context.Background(), testNamespace, sfs, c))
	require.Equal(t, "value2", e2e.GetContentFromURL(varLocalEndpoint, timeout))
	...
	require.NoError(t, e2e.DestroyPod(context.Background(), testNamespace, "app=e2etest", c))
...
func TestUpStatefulsetV2(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestUpStatefulsetV2", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
...
func compareStatefulSet(ctx context.Context, deployment *appsv1.StatefulSet, c kubernetes.Interface) error {
	after, err := e2e.GetStatefulset(ctx, deployment.GetNamespace(), deployment.GetName(), c)
	if err != nil {
		return err
	}
