...
func TestUpGlobalForwarding(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestGlobalFwd", user)
	...
	require.Equal(t, e2e.GetContentFromURL(svc1LocalEndpoint, timeout), "svc1")
	require.Equal(t, e2e.GetContentFromURL(svc2LocalEndpoint, timeout), "svc2")
	require.Equal(t, e2e.GetContentFromURL(svc3LocalEndpoint, timeout), "svc3")
	...
	require.Equal(t, e2e.GetContentFromURL(svc1LocalEndpoint, 20*time.Second), "")
	require.Equal(t, e2e.GetContentFromURL(svc2LocalEndpoint, 5*time.Second), "")
	require.Equal(t, e2e.GetContentFromURL(svc3LocalEndpoint, 5*time.Second), "")
}
