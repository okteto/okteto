...
func TestUpDeploymentV1(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestUpDeploymentV1", user)
	...
	require.NoError(t, e2e.WaitForDeployment(kubectlBinary, kubectlOpts, 1, timeout))

	originalDeployment, err := e2e.GetDeployment(context.Background(), testNamespace, "e2etest", c)
	require.NoError(t, err)
	...
	d, err := e2e.GetDeployment(context.Background(), testNamespace, "e2etest", c)
	d.ResourceVersion = ""
	require.NoError(t, err)
	...
	require.NoError(t, e2e.UpdateDeployment(context.Background(), testNamespace, d, c))
...
func TestUpDeploymentV2(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestUpDeploymentV2", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
...
func compareDeployment(ctx context.Context, deployment *appsv1.Deployment, c kubernetes.Interface) error {
	after, err := e2e.GetDeployment(ctx, deployment.GetNamespace(), deployment.GetName(), c)
	if err != nil {
		return err
	}
