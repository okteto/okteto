...
func TestAutoWakeFromURL(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestAutoWakeURL", user)
	...
	require.NotEmpty(t, e2e.GetContentFromURL(autowakeURL, timeout))
	require.NoError(t, waitUntilUpdatedContent(autowakeURL, "test", timeout))
...
func TestAutoWakeFromRunningUp(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestAutoWakeUp", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
...
func areNamespaceResourcesSleeping(namespace string, timeout time.Duration, c kubernetes.Interface) bool {
	...
	for {
		...
		case <-to.C:
			...
			return false
		case <-ticker.C:
			d, err := e2e.GetDeployment(context.Background(), namespace, "autowake", c)
			if err != nil {
				if retry%10 == 0 {
			...
			sfs, err := e2e.GetStatefulset(context.Background(), namespace, "autowake", c)
			if err != nil {
				if retry%10 == 0 {
...
func areNamespaceResourcesAwake(namespace string, timeout time.Duration, c kubernetes.Interface) bool {
	...
	for {
		...
		case <-ticker.C:
			retry++
			dList, err := e2e.GetDeploymentList(context.Background(), namespace, c)
			if err != nil {
				if retry%10 == 0 {
			...
			sfsList, err := e2e.GetStatefulsetList(context.Background(), namespace, c)
			if err != nil {
				if retry%10 == 0 {
					log.Printf("error getting sfs: %s", err.Error())
				}
...
func waitUntilUpdatedContent(url, expectedContent string, timeout time.Duration) error {
	...
	for {
		...
		case <-ticker.C:
			retry++
			content := e2e.GetContentFromURL(url, contentTimeout)
			if content == "" {
				continue
			}
