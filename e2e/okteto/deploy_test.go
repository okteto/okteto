...
func TestDeploySuccessOutput(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestSuccessOutput", user)
	...
	require.NoError(t, err)
	cmap, err := e2e.GetConfigmap(context.Background(), testNamespace, fmt.Sprintf("okteto-git-%s", filepath.Base(dir)), c)
	require.NoError(t, err)
...
func TestDeployWithNonSanitizedOK(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestDeployWithNonSanitizedOK", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
	...
	c, _, err := okteto.NewK8sClientProvider().Provide(kubeconfig.Get([]string{filepath.Join(dir, ".kube", "config")}))
	require.NoError(t, err)
	_, err = e2e.GetConfigmap(context.Background(), testNamespace, fmt.Sprintf("okteto-git-%s", "test-my-deployment"), c)
	require.NoError(t, err)
...
func TestCmdFailOutput(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestCmdFailOutput", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
...
func TestRemoteMaskVariables(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestRemoteMaskVariables", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
...
func TestComposeFailOutput(t *testing.T) {
	...
	testNamespace := e2e.GetTestNamespace("TestComposeFailOutput", user)
	namespaceOpts := &commands.NamespaceOptions{
		Namespace:  testNamespace,
		OktetoHome: dir,
		Token:      token,
