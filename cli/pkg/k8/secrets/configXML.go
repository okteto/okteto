...
{{ range $key, $value := . }}
    <folder id="cnd-{{ $value.Name }}-{{ $value.Container }}" label="{{ $value.Name }}-{{ $value.Container }}" path="/var/cnd-sync/{{ $value.Container }}" type="sendreceive" rescanIntervalS="3600" fsWatcherEnabled="true" fsWatcherDelayS="1" ignorePerms="false" autoNormalize="true">
