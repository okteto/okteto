...
func Insert(
	...
	namespace string, dev *model.Dev, syncthingURI, pod, manifest string) error {

	if err := insert(namespace, dev, syncthingURI, pod, manifest); err != nil {
		return err
	}
...
func insert(namespace string, dev *model.Dev, syncthingURI, pod, manifest string) error {
	s, err := load()
	if err != nil {
		return err
	}

	fullName := getFullName(namespace, dev.Name, dev.Container)
	svc, err := newService(dev.WorkDir.Source, syncthingURI)
	if err != nil {
		return err
	}
...
func newService(folder, syncthingURI string) (Service, error) {
	absFolder, err := fixPath(folder)
	if err != nil {
		return Service{}, err
	}
	return Service{Folder: absFolder, Syncthing: syncthingURI}, nil
}
