...
{{ range $key, $value := .DevList }}
    <folder id="cnd-{{ $value.Name }}-{{ $value.Container }}" label="{{ $value.Name }}-{{ $value.Container }}" path="{{ $value.WorkDir.Source }}" type="{{if or $value.WorkDir.SendOnly $.ForceSendOnly}}sendonly{{else}}sendreceive{{end}}" rescanIntervalS="3600" fsWatcherEnabled="true" fsWatcherDelayS="1" ignorePerms="false" autoNormalize="true">
