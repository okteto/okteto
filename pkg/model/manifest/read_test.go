  ...
  - name: deployment
    ...
    resources:
      ...
      limits:
        ...
        cpu: "500m"
    `)
	m, err := Read(manifestBytes)
	if err != nil {
		t.Fatal(err)
	}

	main := m.Devs["deployment"]

	if len(main.Services) != 1 {
		t.Errorf("'services' was not parsed: %+v", main)
	}
	for _, dev := range []*dev.Dev{main, main.Services[0]} {
  ...
  - 9001:8001`),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defaultTimeout, _ := dev.GetTimeout()
...
services:
  - name: deployment
    ...
    workdir: /app
    %s`, tt.value))
			expected := fmt.Sprintf("Error on dev 'deployment': %q is not supported in Services.Please visit %s for documentation", tt.name, constants.ManifestSvcsDocsURL)
...
services:
  - name: deployment-$SERVICE
	...
	main := manifest.Devs["deployment-main"]
