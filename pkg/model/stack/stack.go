...
type Service struct {
	...
	Entrypoint dev.Entrypoint       `yaml:"entrypoint,omitempty"`
	Command    dev.Command          `yaml:"command,omitempty"`
	EnvFiles   environment.EnvFiles `yaml:"env_file,omitempty"`
...
//StackVolume represents a stack volume
...
type VolumeSpec struct {
	Labels      metadata.Labels      `yaml:"labels,omitempty"`
	Annotations metadata.Annotations `yaml:"annotations,omitempty"`
	Size        Quantity             `json:"size,omitempty" yaml:"size,omitempty"`
	Class       string               `json:"class,omitempty" yaml:"class,omitempty"`
}

//Envs represents a list of env vars
type Envs struct {
...
//HTTPHealtcheck represents an HTTP healthcheck
type HTTPHealtcheck struct {
	Path string `yaml:"path,omitempty"`
	Port int32  `yaml:"port,omitempty"`
}

//HealtcheckTest represents a list of commands to run
...
//EndpointSpec represents an map of endpoints
...
//StackWarnings saves the warning during unmarshaling
...
func GetStack(name, stackPath string, isCompose bool) (*Stack, error) {
	...
	expandedManifest, err := environment.ExpandEnv(string(b))
	if err != nil {
		return nil, err
	}
	...
	for _, svc := range s.Services {
		...
		if uri, err := url.ParseRequestURI(svc.Build.Context); err == nil || (uri != nil && (uri.Scheme != "" || uri.Host != "")) {
			...
			svc.Build.Context = files.LoadAbsPath(stackDir, svc.Build.Context)
			svc.Build.Dockerfile = files.LoadAbsPath(svc.Build.Context, svc.Build.Dockerfile)
		}
	}
	return s, nil
}
...
func getStackName(name, stackPath, actualStackName string) (string, error) {
	...
	if actualStackName == "" {
		name, err := files.GetValidNameFromGitRepo(filepath.Dir(stackPath))
		if err != nil {
			name, err = files.GetValidNameFromFolder(filepath.Dir(stackPath))
			if err != nil {
				return "", err
			}
		}
...
func ReadStack(bytes []byte, isCompose bool) (*Stack, error) {
	...
	if err := yaml.UnmarshalStrict(bytes, s); err != nil {
		if strings.HasPrefix(err.Error(), "yaml: unmarshal errors:") {
			...
			_, _ = sb.WriteString(fmt.Sprintf("    See %s for details", constants.StackCtxDocsURL))
...
//IgnoreSyncVolumes ignores syncthing volumes
...
func validateStackName(name string) error {
	...
	if files.ValidKubeNameRegex.MatchString(name) {
...
//IsPortInService checks if the port is already on the service
...
func (svc *Service) SetLastBuiltAnnotation() {
	if svc.Annotations == nil {
		svc.Annotations = metadata.Annotations{}
	}
...
//IsAlreadyAddedExpose checks if its already declared a expose port
...
//GroupWarningsBySvc group warning by fields
...
//IsDeployment returns true if service is a deployment
...
//IsStatefulset returns true if service is a sfs
func (svc *Service) IsStatefulset() bool {
	return len(svc.Volumes) != 0 && (svc.RestartPolicy == apiv1.RestartPolicyAlways || (svc.RestartPolicy == apiv1.RestartPolicyOnFailure && svc.BackOffLimit == 0))
}

//IsJob returns true if service is a job
...
//Merge merges two stack
func (s *Stack) Merge(otherStack *Stack) *Stack {
	if s == nil {
		return otherStack
	}
	if otherStack.Namespace != "" {
		s.Namespace = otherStack.Namespace
	}
	if len(otherStack.Endpoints) > 0 {
		s.Endpoints = otherStack.Endpoints
	}
	if len(otherStack.Volumes) > 0 {
		s.Volumes = otherStack.Volumes
	}
	s = s.mergeServices(otherStack)
	return s
}

func (s *Stack) mergeServices(otherStack *Stack) *Stack {
	for svcName, svc := range otherStack.Services {
		if _, ok := s.Services[svcName]; !ok {
			s.Services[svcName] = svc
			continue
		}
		resultSvc := s.Services[svcName]
		if svc.Image != "" {
...
//IsDefaultValue returns true if resources are the default values
...
//IsDefaultValue checks if the resources is default
