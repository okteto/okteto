...
type serviceRaw struct {
	...
	EnvFilesSneakCase        environment.EnvFiles    `yaml:"env_file,omitempty"`
	EnvFiles                 environment.EnvFiles    `yaml:"envFile,omitempty"`
...
func unmarshalVolume(volume *VolumeTopLevel) (*VolumeSpec, error) {
	...
	result := &VolumeSpec{}
	if volume == nil {
		result.Labels = make(metadata.Labels)
		result.Annotations = make(metadata.Annotations)
	} else {
		result.Labels = make(metadata.Labels)
...
//ToService translate serviceRaw to Service
func (serviceRaw *serviceRaw) ToService(svcName string, stack *Stack) (*Service, error) {
	...
	if svc.Annotations == nil {
		svc.Annotations = make(metadata.Annotations)
	}
	...
	if stack.IsCompose {
		...
		if svc.Labels == nil {
			svc.Labels = make(metadata.Labels)
		}
		for key, value := range unmarshalLabels(serviceRaw.Labels, serviceRaw.Deploy) {
	...
	svc.Environment = environment.Environment{}
...
func isNamedVolumeDeclared(volume StackVolume) bool {
	if volume.LocalPath != "" {
		...
		if files.FileExists(volume.LocalPath) {
			return true
		}
	}
	return false
...
func unmarshalLabels(labels metadata.Labels, deployInfo *deployInfoRaw) metadata.Labels {
...
func getPortWithMapping(p *portRaw, portString string) error {
	...
	if len(localToContainer) > 3 {
		return fmt.Errorf(port.MalformedPortForward, portString)
	}
...
func (endpoint *Endpoint) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	err := unmarshal(&rules)
	endpoint.Labels = make(metadata.Labels)
	if err == nil {
	...
	if endpoint.Annotations == nil {
		endpoint.Annotations = make(metadata.Annotations)
	}
