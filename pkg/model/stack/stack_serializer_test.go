...
func Test_validateCommandArgs(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			isCompose:   true,
			Entrypoint:  dev.Entrypoint{Values: []string{"/usr/bin/rpk", "redpanda"}},
			Command:     dev.Command{},
			expectedErr: false,
		},
		{
		...
		},
		{
			...
			isCompose:   true,
			Command:     dev.Command{Values: []string{"/usr/bin/rpk", "redpanda"}},
			Entrypoint:  dev.Entrypoint{Values: []string{"entrypoint.sh"}},
			expectedErr: false,
		},
		{
...
func Test_unmarshalVolumes(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			manifest:       []byte("services:\n  app:\n    image: okteto/vote:1\nvolumes:\n  v1:\n    labels:\n      env: test"),
			expectedVolume: &VolumeSpec{Size: Quantity{resource.MustParse("1Gi")}, Labels: metadata.Labels{}, Annotations: metadata.Annotations{"env": "test"}},
		},
		{
...
func Test_DeployLabels(t *testing.T) {
	tests := []struct {
...

		{
			name:        "deploy labels",
			manifest:    []byte("services:\n  app:\n    deploy:\n      labels:\n        env: production\n    image: okteto/vote:1"),
			annotations: metadata.Annotations{"env": "production"},
			labels:      metadata.Labels{},
			isCompose:   true,
		},
		{
		...
		},
		{
			...
			manifest:    []byte("services:\n  app:\n    image: okteto/vote:1\n    labels:\n      app: main\n    deploy:\n      labels:\n        env: production\n"),
			annotations: metadata.Annotations{"env": "production", "app": "main"},
			labels:      metadata.Labels{},
			isCompose:   true,
		},

		{
			...
			manifest:    []byte("services:\n  app:\n    image: okteto/vote:1\n    labels:\n      app: main\n    deploy:\n      labels:\n        env: production\n"),
			annotations: metadata.Annotations{},
			labels:      metadata.Labels{"app": "main", "env": "production"},
			isCompose:   false,
		},
		{
			name:        "labels on service",
			manifest:    []byte("services:\n  app:\n    image: okteto/vote:1\n    labels:\n      env: production"),
			annotations: metadata.Annotations{},
			labels:      metadata.Labels{"env": "production"},
			isCompose:   false,
		},
	}
	for _, tt := range tests {
  ...
  app:
    ...
    - path: /
      ...
      port: 9213`),
			expected: EndpointSpec{
				"app": Endpoint{
					Annotations: metadata.Annotations{"key": "value"},
					Labels:      metadata.Labels{},
					Rules: []EndpointRule{
						{
							Service: "app",
...
func Test_validateEnvFiles(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			manifest: []byte("services:\n  app:\n    env_file: .env\n    public: true\n    image: okteto/vote:1"),
			EnvFiles: environment.EnvFiles{".env"},
		},
		{
			name:     "sneak case list",
			manifest: []byte("services:\n  app:\n    env_file:\n    - .env\n    - .env2\n    image: okteto/vote:1"),
			EnvFiles: environment.EnvFiles{".env", ".env2"},
		},
		{
...
func Test_Environment(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:        "envs",
			manifest:    []byte("services:\n  app:\n    environment:\n        env: production\n    image: okteto/vote:1"),
			environment: environment.Environment{environment.EnvVar{Name: "env", Value: "production"}},
		},
		{
			name:        "empty envs",
			manifest:    []byte("services:\n  app:\n    environment:\n      - env\n    image: okteto/vote:1"),
...
func TestEndpointUnmarshalling(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: Endpoint{
				Labels:      metadata.Labels{},
				Annotations: metadata.Annotations{"key1": "value1", "key2": "value2"},
				Rules: []EndpointRule{{
					Path:    "/",
					Service: "test",
					Port:    8080,
		...
		},
		{
			...
			expected: Endpoint{
				Labels:      metadata.Labels{},
				Annotations: metadata.Annotations{"key2": "value2"},
				Rules: []EndpointRule{{
					Path:    "/",
					Service: "test",
					Port:    8080,
