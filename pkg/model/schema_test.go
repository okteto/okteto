	...
	tests := []struct {
		...
		},
		{
			...
			expected: map[string][]string{
				...
				"model.Dev":                  {"resources", "selector", "persistentVolume", "securityContext", "labels", "probes", "nodeSelector", "metadata", "affinity", "image", "lifecycle", "replicas", "initContainer", "workdir", "name", "context", "namespace", "container", "serviceAccount", "interface", "mode", "imagePullPolicy", "tolerations", "command", "forward", "reverse", "externalVolumes", "secrets", "volumes", "envFiles", "environment", "services", "args", "sync", "timeout", "remote", "sshServerPort", "autocreate"},
