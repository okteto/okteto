...
//ExpandEnvWithDefaults expands the envvaars in the given string
func ExpandEnvWithDefaults(value string) string {
	mapper := func(placeholderName string) string {
		parts := strings.Split(placeholderName, ":-")
		switch len(parts) {
		case 0:
		...
		default:
			eValue, ok := os.LookupEnv(parts[0])
			if ok {
				return eValue
			}
			return parts[1]
