...
services:
  ...
  db:
	...
	if len(s.Services["vote"].Env) != 2 {
		t.Errorf("'vote.env' was not parsed: %+v", s)
	}
	if s.Services["vote"].Env[0].Name != "OPTION_A" || s.Services["vote"].Env[0].Value != "Cats" {
		t.Errorf("'vote.env[0]' was not parsed: %+v", s)
	}
	if s.Services["vote"].Env[1].Name != "OPTION_B" || s.Services["vote"].Env[1].Value != "Dogs" {
