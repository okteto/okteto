...
//MergeDevWithDevRc merges a devRC with a dev manifest
...
func getEnvVarIdx(environment environment.Environment, envVar environment.EnvVar) int {
	idx := -1
...
func getForwardPortIdx(forwardList []port.Forward, forward port.Forward) int {
	idx := -1
