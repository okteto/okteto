...
func TestValidateEndpoints(t *testing.T) {
			...
			Ports: []apiv1.ServicePort{
				{
					...
					TargetPort: intstr.IntOrString{IntVal: int32(externalPort)},
				},
			},
		},
	}
	...
	tests := []struct {
		...
		{
			name: "endpoints-svcs-not-defined-on-stack",
			stack: &model.Stack{
				Namespace: "test",
				Services: map[string]*model.Service{
					"api": {
						Image: "hello-world",
						Ports: []model.Port{
							{Port: 5000, Protocol: apiv1.ProtocolTCP},
						},
					},
				},
		...
		{
			name: "endpoints-svcs-defined-on-stack",
			stack: &model.Stack{
				Namespace: "test",
				Services: map[string]*model.Service{
					"api": {
		...
		{
			name: "endpoints-svcs-not-defined-on-stack-or-external",
			stack: &model.Stack{
				Namespace: "test",
				Services: map[string]*model.Service{
					"api": {
		...
		{
			name: "endpoints-svcs-defined-on-stack-bad-port",
			stack: &model.Stack{
				Namespace: "test",
				Services: map[string]*model.Service{
					"api": {
		...
		{
			name: "endpoints-svcs-not-defined-on-stack-bad-port",
			stack: &model.Stack{
				Namespace: "test",
				Services: map[string]*model.Service{
					"api": {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := validateEndpoints(ctx, tt.stack, clientset)
			if err == nil && tt.expectedError {
				t.Fatal("Expected error but none error found")
			}
			if err != nil && !tt.expectedError {
				t.Fatal(err)
			}
