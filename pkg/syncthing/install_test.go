...
func TestGetMinimumVersion(t *testing.T) {
	...
	env := os.Getenv(constants.SyncthingVersionEnvVar)
	if err := os.Setenv(constants.SyncthingVersionEnvVar, ""); err != nil {
		t.Fatal(err)
	}

	defer func() {
		if err := os.Setenv(constants.SyncthingVersionEnvVar, env); err != nil {
			t.Fatal(err)
		}
	}()
	...
	for _, tt := range tests {
		t.Run(tt.version, func(t *testing.T) {
			if err := os.Setenv(constants.SyncthingVersionEnvVar, tt.version); err != nil {
				t.Fatal(err)
			}
