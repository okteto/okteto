...
func TestForward(t *testing.T) {
	...
	sshPort, err := model.GetAvailablePort(constants.Localhost)
	if err != nil {
		t.Fatal(err)
	}
...
func startServers(fm *ForwardManager) error {
	for i := 0; i < 1; i++ {
		local, err := model.GetAvailablePort(constants.Localhost)
		if err != nil {
			return err
		}

		remote, err := model.GetAvailablePort(constants.Localhost)
		if err != nil {
			return err
		}
