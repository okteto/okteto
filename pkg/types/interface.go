...
type PreviewInterface interface {
	...
	DeployPreview(ctx context.Context, name, scope, repository, branch, sourceUrl, filename string, variables []Variable, labels []string, redeployDependencies bool, workflowID string) (*PreviewResponse, error)
...
type KubetokenInterface interface {
	GetKubeToken(baseURL, namespace, target string) (KubeTokenResponse, error)
