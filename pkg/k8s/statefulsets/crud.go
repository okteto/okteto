...
func Destroy(dev *model.Dev, c *kubernetes.Clientset) error {
	...
	log.Infof("destroying syncthing statefulset '%s' ...", name)
	sfsClient := c.AppsV1().StatefulSets(dev.Namespace)
	if err := sfsClient.Delete(name, &metav1.DeleteOptions{GracePeriodSeconds: &terminationGracePeriodSeconds}); err != nil {
