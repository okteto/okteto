...
func (a *Arg) String() string {
	value, err := vars.GlobalVarManager.Expand(a.Value)
	if err != nil {
...
func (a *Arg) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	a.Name, err = vars.GlobalVarManager.Expand(parts[0])
	if err != nil {
		return err
	}
	a.Value = parts[0]
...
func getArgs(unmarshal func(interface{}) error) (map[string]string, error) {
	...
	if err == nil {
		for _, buildArg := range rawList {
			value, err := vars.GlobalVarManager.ExpandIfNotEmpty(buildArg.Value)
			if err != nil {
				return nil, err
			}
	...
	for key, value := range rawMap {
		result[key], err = vars.GlobalVarManager.ExpandIfNotEmpty(value)
		if err != nil {
			return nil, err
		}
	}
