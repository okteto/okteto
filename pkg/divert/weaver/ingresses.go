...
func (d *driver) divertIngress(ctx context.Context, name string) error {
	...
	if !ok {
		in = translateIngress(d.m, from)
		oktetoLog.Infof("creating ingress %s/%s", in.Namespace, in.Name)
		if _, err := d.c.NetworkingV1().Ingresses(d.m.Namespace).Create(ctx, in, metav1.CreateOptions{}); err != nil {
		...
		if in.Annotations[model.OktetoAutoCreateAnnotation] == "true" {
			...
			updatedIn = translateIngress(d.m, d.cache.divertIngresses[name])
		} else if in.Annotations[model.OktetoDivertIngressInjectionAnnotation] != d.m.Namespace {
			...
			updatedIn.Annotations[model.OktetoDivertIngressInjectionAnnotation] = d.m.Namespace
			updatedIn.Annotations[model.OktetoNginxConfigurationSnippetAnnotation] = divertTextBlockParser.WriteBlock(fmt.Sprintf("proxy_set_header x-okteto-dvrt %s;", d.m.Namespace))
		}
		if !isEqualIngress(in, updatedIn) {
			oktetoLog.Infof("updating ingress %s/%s", updatedIn.Namespace, updatedIn.Name)
			if _, err := d.c.NetworkingV1().Ingresses(d.m.Namespace).Update(ctx, updatedIn, metav1.UpdateOptions{}); err != nil {
