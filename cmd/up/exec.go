...
func (up *upContext) checkOktetoStartError(ctx context.Context, msg string) error {
	...
	After that, run '%s' to reset your development container and run 'okteto up' again`, userID, utils.GetDownCommand(up.Options.OriginalManifestPath)),
			}
		}
	}
	...
	Run '%s' to reset your development container and try again`, up.Pod.Name, utils.GetDownCommand(up.Options.OriginalManifestPath)),
		}
	}
	return oktetoErrors.UserError{
		E: fmt.Errorf(msg),
