...
func (pc *Command) ExecuteDeployPipeline(ctx context.Context, opts *DeployOptions) error {
	...
	if err := pc.setEnvsFromDependency(ctx, opts.Name, opts.Namespace, c); err != nil {
		return fmt.Errorf("could not set environment variable generated by dependency '%s'", opts.Name)
	}
...
func (pc *Command) setEnvsFromDependency(ctx context.Context, name, namespace string, c kubernetes.Interface) error {
