...
func (pc *Command) ExecuteDeployPipeline(ctx context.Context, opts *DeployOptions) error {
	...
	if err := setVarsFromDependency(cmap, pc.varManager); err != nil {
...
func setVarsFromDependency(cmap *v1.ConfigMap, varManager *vars.Manager) error {
	...
	dependencyVarsEncoded, ok := cmap.Data[constants.OktetoDependencyEnvsKey]
	if !ok {
		return nil
	}
	...
	decodedVars, err := base64.StdEncoding.DecodeString(dependencyVarsEncoded)
	if err != nil {
		return err
	}
