...
func (ob *OktetoBuilder) newServiceContext(buildInfo *model.BuildInfo, repoService repository.RepositoryService) serviceContextChecker {
	service := buildInfo.Name
	isClean, err := repoService.IsBuildContextClean(buildInfo.Context)
	if err != nil {
		oktetoLog.Infof("failed to check if service '%s' has build context clean: %w", service, err)
	}
	ob.lock.Lock()
...
func (bc *OktetoBuilder) Build(ctx context.Context, options *types.BuildOptions) error {
	...
	for len(bc.builtImages) != len(toBuildSvcs) {
		for _, svcToBuild := range toBuildSvcs {
			...
			serviceController := repository.NewRepositoryService(buildSvcInfo.Name)

			serviceContextChecker := bc.newServiceContext(buildSvcInfo, serviceController)
			...
			if !options.NoCache && serviceContextChecker.IsCleanBuildContext() {
				...
				if imageWithDigest, isBuilt := imageChecker.checkIfCommitHashIsBuilt(options.Manifest.Name, svcToBuild, buildSvcInfo); isBuilt {
					...
					if bc.Registry.IsGlobalRegistry(imageWithDigest) {
						...
						tag := serviceContextChecker.GetBuildHash()
...
func getImageChecker(buildInfo *model.BuildInfo, cfg oktetoBuilderConfigInterface, svcCfg serviceContextInterface, registry registryImageCheckerInterface) imageCheckerInterface {
	...
	return newImageChecker(cfg, registry, tagger, svcCfg)
}
