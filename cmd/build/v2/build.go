...
func NewBuilder(builder buildCmd.OktetoBuilderInterface, registry oktetoRegistryInterface, ioCtrl *io.Controller, okCtx okteto.ContextInterface, k8sLogger *io.K8sLogger, varManager *vars.Manager, onBuildFinish []OnBuildFinish) *OktetoBuilder {
	...
	return &OktetoBuilder{
		...
		Config:            config,
		ioCtrl:            ioCtrl,
		smartBuildCtrl:    smartbuild.NewSmartBuildCtrl(gitRepo, registry, config.fs, ioCtrl, varManager),
		oktetoContext:     okCtx,
		k8sLogger:         k8sLogger,
...
func NewBuilderFromScratch(ioCtrl *io.Controller, varManager *vars.Manager, onBuildFinish []OnBuildFinish) *OktetoBuilder {
	...
	return &OktetoBuilder{
		...
		ioCtrl:            ioCtrl,
		varManager:        varManager,
		smartBuildCtrl:    smartbuild.NewSmartBuildCtrl(gitRepo, reg, config.fs, ioCtrl, varManager),
		oktetoContext:     okCtx,
...
func (bc *OktetoBuilder) buildSvcFromDockerfile(ctx context.Context, manifest *model.Manifest, svcName string, options *types.BuildOptions) (string, error) {
	...
	buildOptions := buildCmd.OptsFromBuildInfo(manifest.Name, svcName, buildSvcInfo, options, bc.Registry, bc.oktetoContext, bc.varManager)
