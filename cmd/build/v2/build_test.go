...
func TestOnlyInjectVolumeMountsInOkteto(t *testing.T) {
	...
	image, err := bc.buildService(ctx, &fakeServiceContext{}, manifest, "test", &types.BuildOptions{})
...
func TestGenerateHashWithBuildInfo(t *testing.T) {
	...
	tt := []struct {
	...
	}{
		{
			...
			expected: "context_tree_hash:1234567890;target:target;build_args:foo=bar;key=value;secrets:secret=secret;context:context;dockerfile:dockerfile;image:image;",
		},
		{
			name: "invalid commit",
			input: input{
			...
			expected: "context_tree_hash:;target:target;build_args:foo=bar;key=value;secrets:secret=secret;context:context;dockerfile:dockerfile;image:image;",
		},
		{
			...
			expected: "context_tree_hash:;target:target;build_args:;secrets:secret=secret;context:context;dockerfile:dockerfile;image:image;",
		},
		{
			...
			expected: "context_tree_hash:;target:target;build_args:foo=bar;secrets:secret=secret;context:context;dockerfile:dockerfile;image:image;",
		},
	}
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			assert.Equal(t, tc.expected, getBuildHashToEncode(tc.input.sha, tc.input.buildInfo))
		})
	}
}
